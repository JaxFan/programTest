 ...
 * 实现原理：
 * V1.0
 * 通过动态malloc节点+双向链表形式组成定时器组。
 * 增加删除时，按照从进到远的超时绝对时间排列，通过依次遍历链表找到增加的节点。（可升级）
 * 定时器裁决线程：从头到尾遍历一遍为一个周期。如果遇到的第一个timer都没超期，这说明后面的都不可能超期。直接break
 * TODO
 * 1.可以考虑动态，静态两种初始化方式。
 * 定时器的存储可以用小根堆吗？
 * 当前定时器只有秒的精度，无法设置成100ms
...
/*
 * 定时器链表数据结构定义
 * 定时器最大支持32种不同的超时时间，每个数组元素代表一个超时时间，下挂一条双向链表(不是1-32S 是32种！ 9999S也只是一种)
